...
async def create_pet_for_user(pet: PetCreate,
    ...
    crud_base.create_user_pet(pet, user_id, db)
...
async def show_pet(pet_id: int = Query(..., description="id питомца"),
    ...
    pet = crud_base.get_pet(owner_id, pet_id, db)
    crud_base.check_for_existence_in_db(pet, f"Питомец по id хозяина = {owner_id} и id животного = {pet_id} не найден")
...
async def show_all_pets(skip: int = Query(0, description="Сколько записей пропустить"),
    ...
    all_pets = crud_base.get_entries(PetModel, db, skip, limit)
    crud_base.check_for_existence_in_db(all_pets, "База данных питомцев пуста")
...
async def change_pet(pet_id: int = Path(..., description="id питомца"),
    ...
    user = crud_base.get_user(owner_id, db)
    crud_base.check_for_existence_in_db(user, f"Пользователь с id {owner_id} не найден")
    user = crud_base.get_pet(owner_id, pet_id, db)
    crud_base.check_for_existence_in_db(user, f"Питомец с таким id у данного пользователя не найден")
    modified_pet = crud_base.get_pets_by_animal_name_and_description(new_animal_name, new_description, db)
    crud_base.checking_for_matches_in_db(modified_pet,
                                         "У пользователя уже есть питомец с такой кличкой и описанием")
    pet = crud_base.get_pet(owner_id, pet_id, db)
    crud_base.put_pet(pet, new_animal_name, new_description, db)
...
async def delete_pet(pet_id: int = Path(..., description="id питомца"),
    ...
    pet_to_be_deleted = crud_base.get_pet(owner_id, pet_id, db)
    crud_base.check_for_existence_in_db(pet_to_be_deleted,
                                   f"Питомец с id хозяина = {owner_id} и id животного = {pet_id} не найден")
    crud_base.delete_entry(pet_to_be_deleted, db)

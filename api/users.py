...
async def create_user(new_user: UserCreate, db: Session = Depends(get_db)):
    ...
    user_by_new_email = crud_base.get_user_by_email(new_user.email, db)
    crud_base.checking_for_matches_in_db(user_by_new_email, "Пользователь с таким email уже зарегистрирован")
    return crud_base.create_user(new_user, db)
...
async def show_users(skip: int = Query(0, description="Сколько записей пропустить"),
    ...
    users = crud_base.get_entries(UserModel, db, skip, limit)
    crud_base.check_for_existence_in_db(users, "База данных пользователей пуста")
...
async def show_user(user_id: int = Path(..., description="Пользовательский id"),
    ...
    user = crud_base.get_user(user_id, db)
    crud_base.check_for_existence_in_db(user, f"Пользователь с id {user_id} не найден")
...
async def change_user_by_id(user_id: int = Path(..., description="Пользовательский id"),
    ...
    user = crud_base.get_user(user_id, db)
    crud_base.check_for_existence_in_db(user, f"Пользователь с id {user_id} не найден")
    user_with_the_same_email = crud_base.get_user_by_email(new_email, db)
    crud_base.checking_for_matches_in_db(user_with_the_same_email, f"Пользователь с email: {new_email} уже зарегистрирован")
    user = crud_base.get_user(user_id, db)
    crud_base.put_user(user, new_email, db)
...
async def delete_user(user_id: int = Path(..., description="id удаляемого пользователя"),
    ...
    all_pets_from_user = crud_base.get_all_pets_from_user(user_id, db)
    crud_base.delete_entries(all_pets_from_user, db)
    user_to_be_deleted = crud_base.get_user(user_id, db)
    crud_base.check_for_existence_in_db(user_to_be_deleted, f"Пользователь с id = {user_id} не найден")
    crud_base.delete_entry(user_to_be_deleted, db)
...
async def delete_all_users(db: Session = Depends(get_db)):
    ...
    all_pets = crud_base.get_entries(PetModel, db, display_all=True)
    crud_base.check_for_existence_in_db(all_pets, "База данных питомцев пуста", exception=False)
    crud_base.delete_entries(all_pets, db)
    all_users = crud_base.get_entries(UserModel, db, display_all=True)
    crud_base.check_for_existence_in_db(all_users, "База данных пользователей пуста")
    crud_base.delete_entries(all_users, db)
